...
from qdp_tdgl.config.loaders import load_case_config, tdgl_config_root
from qdp_tdgl.exceptions import ManifestDispatchError, ManifestValidationError, TDGLRFError
from qdp_tdgl.workflows.convergence import run_convergence
from qdp_tdgl.workflows.evidence import build_evidence_bundle
from qdp_tdgl.workflows.experiment_sweep import (
    run_seeded_vortex_experiment_pack,
)
from qdp_tdgl.workflows.postprocess import summarize_campaign
from qdp_tdgl.workflows.refinement import run_refinement_sanity
from qdp_tdgl.workflows.run_case import run_simulation
from qdp_tdgl.workflows.run_ensemble import run_ensemble
from qdp_tdgl.workflows.run_inference import run_inference
from qdp_tdgl.workflows.run_matrix import run_experiment_matrix
from qdp_tdgl.workflows.validation import (
...
from qdp_tdgl.workflows.phase2_4a_validation import (
    default_phase2_4a_validation_manifest_path,
...
@app.command("validate-phase2-4a")
def validate_phase2_4a(
    manifest_path: Path = typer.Argument(default_phase2_4a_validation_manifest_path()),
    output_dir: Path | None = typer.Option(None, "--output-dir"),
) -> None:
    """Validate the committed Phase-2.4A fixed-seed ensemble surface."""

    try:
        summary = run_phase2_4a_validation(manifest_path, output_dir=output_dir)
    except Exception as exc:
        _fail(exc)
        return
    typer.echo(json.dumps(asdict(summary), indent=2))
