...
from qdp_meta_materials.adapters.tdgl import (
...
from qdp_tdgl.exceptions import ConfigError, SeedRejectionCode, SeedRejectionError
from qdp_tdgl.geometry.masks import build_geometry, grid_from_config
from qdp_tdgl.runtime_capabilities import require_noise_runtime_support
from qdp_tdgl.solvers.seeded_vortices import resolve_vortex_seeds
...
def _resolve_material_registry_root(materials_payload: dict[str, Any], config_dir: Path) -> Path | None:
    registry_root = materials_payload.get("registry_root")
    if not registry_root:
        return None
    candidate = Path(str(registry_root))
    if candidate.is_absolute():
        return candidate.resolve()
    return (config_dir / candidate).resolve()


def _resolve_material_semantics(data: dict[str, Any], config_dir: Path) -> dict[str, Any]:
    resolved = deepcopy(data)
    physics_payload = resolved.get("physics")
    if not isinstance(physics_payload, dict):
        raise ConfigError("physics must be a mapping")

    missing_physics = [
        field_name
        for field_name in ("u", "sigma_n", "alpha_background")
        if field_name not in physics_payload
    ]
    materials_payload = resolved.get("materials")
    if materials_payload is None:
        if missing_physics:
            raise ConfigError(
                "physics.u, physics.sigma_n, and physics.alpha_background are required unless "
                "materials.adapter_id references the canonical qdp_meta_materials TDGL adapter registry"
            )
        return resolved

    if not isinstance(materials_payload, dict):
        raise ConfigError("materials must be a mapping when provided")

    adapter_id = materials_payload.get("adapter_id")
    material_system_id = materials_payload.get("material_system_id")
    if not adapter_id:
        raise ConfigError("materials.adapter_id is required when materials are provided")
    if not material_system_id:
        raise ConfigError("materials.material_system_id is required when materials are provided")

    geometry_payload = resolved.get("geometry")
    geometry_family = geometry_payload.get("family") if isinstance(geometry_payload, dict) else None
    try:
        resolved_material = resolve_tdgl_material_reference(
            str(adapter_id),
            registry_root=_resolve_material_registry_root(materials_payload, config_dir),
            material_system_id=str(material_system_id),
            genome_id=(
                str(materials_payload["genome_id"])
                if materials_payload.get("genome_id") is not None
                else None
            ),
    ...
    if missing_physics:
        physics_payload["u"] = resolved_material.tdgl_parameters.u
        physics_payload["sigma_n"] = resolved_material.tdgl_parameters.sigma_n
        physics_payload["alpha_background"] = resolved_material.tdgl_parameters.alpha_background
    elif not tdgl_parameters_match(
        observed_u=float(physics_payload["u"]),
        observed_sigma_n=float(physics_payload["sigma_n"]),
        observed_alpha_background=float(physics_payload["alpha_background"]),
        resolved=resolved_material,
    ):
        raise ConfigError(
            "TDGL material reference does not match the explicit physics parameter copy. "
            "Remove the duplicate copy or update it to match the canonical qdp_meta_materials adapter."
        )

    materials_payload.setdefault("registry_source", "qdp_meta_materials.seed.stable")
    materials_payload["adapter_version"] = resolved_material.adapter_version
    materials_payload["material_system_id"] = resolved_material.material_system_id
    materials_payload["material_system_name"] = resolved_material.material_system_name
    materials_payload["runtime_family"] = resolved_material.runtime_family
    materials_payload["source_artifacts"] = list(resolved_material.source_artifacts)
    materials_payload["supported_geometry_families"] = list(
        resolved_material.supported_geometry_families
    )
    materials_payload["resolved_tdgl_parameters"] = resolved_material.tdgl_parameters.model_dump(
        mode="json"
    )
    materials_payload["notes"] = resolved_material.notes
    if resolved_material.genome_id is not None:
        materials_payload["genome_id"] = resolved_material.genome_id
    return resolved
...
def validate_case_config(data: dict[str, Any], schema_path: Path, config_dir: Path) -> TDGLRFCaseConfig:
    ...
    normalized_for_model = _resolve_material_semantics(normalized_for_model, config_dir)
    try:
