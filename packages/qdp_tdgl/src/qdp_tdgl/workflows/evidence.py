...
from qdp_tdgl.config.loaders import repo_root
from qdp_tdgl.exceptions import ConfigError
from qdp_tdgl.workflows.run_matrix import load_experiment_matrix
from qdp_tdgl.workflows.validation import load_threshold_spec
...
def _bundle_overview_markdown(
    *,
    ...
    lines = [
        ...
        "  `qdp-tdgl validate-phase1 matrices/phase1_validation_matrix_v1.csv configs/validation/tdgl/thresholds.yaml configs/validation/tdgl/reference_manifest.yaml configs/tdgl/phase1_refinement_sanity.yaml`",
        "- Package a completed validation run into this bundle form:",
        f"  `qdp-tdgl evidence-bundle {validation_dir}`",
        "",
    ]
    return "\n".join(lines)
