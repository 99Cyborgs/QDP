...
from qdp_tdgl.exceptions import AggregationError
...
def compute_robustness_summary(
    *,
    ...
    for observable_name in observables:
        ...
        coefficient_of_variation_value: float | None
        coefficient_of_variation_defined = True
        try:
            coefficient_of_variation_value = coefficient_of_variation(values)
        except AggregationError as exc:
            if "zero-mean series" not in str(exc):
                raise
            coefficient_of_variation_value = None
        ...
        observable_summary[observable_name] = OrderedDict(
            [
                ...
                ("coefficient_of_variation_defined", coefficient_of_variation_defined),
