...
PACKAGE_SRC = ROOT / "packages" / "qdp_tdgl" / "src"
...
from qdp_tdgl.cli import app
from qdp_tdgl.testing.case_configs import write_case_config
from qdp_tdgl.workflows.phase2_4a_validation import Phase24AValidationSummary


def test_validate_config_cli() -> None:
    runner = CliRunner()
    config_path = ROOT / "configs" / "tdgl" / "d01_smoke.yaml"
    result = runner.invoke(app, ["validate-config", str(config_path)])
    assert result.exit_code == 0
    assert '"case_id": "D01_smoke"' in result.stdout
    assert '"adapter_id": "TDGL-MAT-001-BASELINE-V1"' in result.stdout
        ...
        yaml.safe_dump(
            {
                ...
                "forcing": {
                    ...
                    "omega": 0.0,
                    "phase": 0.0,
                    "rf_profile": "uniform_y",
                    "rf_profile_file": None,
                },
...
def test_refinement_sanity_cli(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        app,
        ["refinement-sanity", str(config_path), "--output-dir", str(tmp_path / "refinement_outputs")],
    )
...
def test_validate_phase2_4a_cli_uses_committed_default_manifest(monkeypatch, tmp_path: Path) -> None:
    captured: dict[str, Path | None] = {}

    def _fake_run(manifest_path, *, output_dir=None):
        captured["manifest_path"] = Path(manifest_path)
        captured["output_dir"] = Path(output_dir) if output_dir is not None else None
        return Phase24AValidationSummary(
            status="success",
            manifest_path=str(captured["manifest_path"]),
            experiment_pack_path=str(
                ROOT / "configs" / "validation" / "tdgl" / "seeded_vortex_phase2_4a_experiment_pack.yaml"
            ),
            output_dir=str(tmp_path / "validation_out"),
            validation_json_path=str(tmp_path / "validation_out" / "phase2_4a_validation.json"),
            validation_markdown_path=str(tmp_path / "validation_out" / "phase2_4a_validation.md"),
            validation_cases_csv_path=str(tmp_path / "validation_out" / "phase2_4a_validation_cases.csv"),
            experiment_results_json_path=str(tmp_path / "validation_out" / "experiment_run" / "experiment_results.json"),
            experiment_results_markdown_path=str(tmp_path / "validation_out" / "experiment_run" / "experiment_results.md"),
            parameter_point_count=2,
            member_count=6,
            mismatch_count=0,
        )

    monkeypatch.setattr("qdp_tdgl.cli.run_phase2_4a_validation", _fake_run)
    runner = CliRunner()

    result = runner.invoke(app, ["validate-phase2-4a", "--output-dir", str(tmp_path / "explicit_out")])

    assert result.exit_code == 0
    assert captured["manifest_path"] == (
        ROOT / "configs" / "validation" / "tdgl" / "seeded_vortex_phase2_4a_validation_manifest.yaml"
    ).resolve()
    assert captured["output_dir"] == (tmp_path / "explicit_out").resolve()
    assert '"status": "success"' in result.stdout
