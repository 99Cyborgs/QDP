...
SCHEMA_PATH = ROOT / "configs" / "tdgl" / "tdgl_case.schema.json"
...
def test_config_accepts_legacy_noise_aliases_when_noise_is_disabled(tmp_path: Path) -> None:
    ...
    config = validate_case_config(payload, SCHEMA_PATH, tmp_path)
...
def test_config_resolves_material_reference_into_runtime_physics(tmp_path: Path) -> None:
    payload = {
        "base_config": None,
        "metadata": {"case_id": "materials_resolved", "phase": "D", "version": "0.1.0"},
        "mesh": {"nx": 16, "ny": 8, "lx": 8.0, "ly": 4.0, "periodic_x": False, "periodic_y": False},
        "geometry": {"family": "strip", "moats": [], "holes": [], "mask_file": None},
        "materials": {
            "registry_source": "qdp_meta_materials.seed.stable",
            "adapter_id": "TDGL-MAT-001-BASELINE-V1",
    ...
    assert config.materials.material_system_name == "Al on high-resistivity silicon"
    assert config.materials.resolved_tdgl_parameters is not None
    assert config.materials.resolved_tdgl_parameters.u == 5.79


def test_config_rejects_material_reference_parameter_drift(tmp_path: Path) -> None:
    payload = {
        "base_config": None,
        "metadata": {"case_id": "materials_mismatch", "phase": "D", "version": "0.1.0"},
        "mesh": {"nx": 16, "ny": 8, "lx": 8.0, "ly": 4.0, "periodic_x": False, "periodic_y": False},
        "geometry": {"family": "strip", "moats": [], "holes": [], "mask_file": None},
        "materials": {
            "registry_source": "qdp_meta_materials.seed.stable",
    ...
    with pytest.raises(ConfigError, match="material reference does not match"):
        validate_case_config(payload, SCHEMA_PATH, tmp_path)


def test_provenance_records_material_registry_identity(tmp_path: Path) -> None:
    payload = {
        "base_config": None,
        "metadata": {"case_id": "materials_provenance", "phase": "D", "version": "0.1.0"},
        "mesh": {"nx": 16, "ny": 8, "lx": 8.0, "ly": 4.0, "periodic_x": False, "periodic_y": False},
        "geometry": {"family": "strip", "moats": [], "holes": [], "mask_file": None},
        "materials": {
            "registry_source": "qdp_meta_materials.seed.stable",
    ...
    config = validate_case_config(payload, SCHEMA_PATH, tmp_path)
    provenance = build_provenance(config, ROOT)

    assert provenance["materials"]["adapter_id"] == "TDGL-MAT-001-BASELINE-V1"
    assert provenance["materials"]["material_system_id"] == "MAT-001"
    assert provenance["materials"]["material_system_name"] == "Al on high-resistivity silicon"
    assert provenance["materials"]["resolved_tdgl_parameters"]["u"] == 5.79
    assert provenance["materials"]["registry_root"].endswith(
        str(Path("packages") / "qdp_meta_materials" / "data" / "seed" / "stable")
    )
