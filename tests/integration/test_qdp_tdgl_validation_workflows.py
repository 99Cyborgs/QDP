...
from qdp_tdgl.testing.case_configs import base_case_payload, read_csv_rows, write_case_config
from qdp_tdgl.workflows.run_case import run_simulation
from qdp_tdgl.workflows.validation import _canonical_sha256, run_phase1_validation, run_reference_check, run_reproducibility_check


def _write_base_config(path: Path) -> None:
    payload = base_case_payload("validation_base", path.parent / "runs")
    payload["output"]["root_dir"] = str((path.parent / "runs").resolve())
    ...
    payload_sha256 = _canonical_sha256(
        {
            ...
            "final_observables": hashed_final_observables,
        }
    )
    return {
        ...
        "command": f"qdp-tdgl run-case {config_path}",
        "expected_files": [
...
def test_run_reproducibility_check_writes_pass_report(tmp_path: Path) -> None:
    ...
    summary = run_reproducibility_check(
        config_path,
        tolerances_path=thresholds_path,
