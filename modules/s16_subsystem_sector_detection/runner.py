#!/usr/bin/env python3
"""
QDP subsystem S16 slow-sector detection module.
"""
...
from tools.workflow.qdp_runtime.qdp_subsystem import get_case, load_matrix, run_selftests as run_subsystem_selftests, sector_detection_payload, summarize_payload


MODULE_PATHS = MODULES["s16"]
...
def run_subsystem_module(case_id: str, matrix_path: Path = GKSL_SUBSYSTEM_CASE_MATRIX) -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    payload = sector_detection_payload(case, MODULE_PATHS["module_id"])
    report = {
        "artifact_id": "QDP_V10_6_S16_REPORT",
...
def run_selftests(cases_path: Path, matrix_path: Path, schema_path: Path, output_dir: Path, write_report_path: Path) -> Dict[str, Any]:
    return run_subsystem_selftests(
        ...
        payload_builder=sector_detection_payload,
    )


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the QDP subsystem S16 slow-sector detection module.")
    ...
    if not args.case_id:
        raise SystemExit("S16 run requires --case-id unless --selftest is set.")
