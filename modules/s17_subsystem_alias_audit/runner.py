#!/usr/bin/env python3
"""
QDP subsystem S17 alias and null-baseline audit module.
"""
...
from tools.workflow.qdp_runtime.qdp_subsystem import alias_audit_payload, get_case, load_matrix, run_selftests as run_subsystem_selftests, summarize_payload


MODULE_PATHS = MODULES["s17"]
...
def run_subsystem_module(case_id: str, matrix_path: Path = GKSL_SUBSYSTEM_CASE_MATRIX) -> tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    payload = alias_audit_payload(case, MODULE_PATHS["module_id"])
    report = {
        "artifact_id": "QDP_V10_6_S17_REPORT",
...
def run_selftests(cases_path: Path, matrix_path: Path, schema_path: Path, output_dir: Path, write_report_path: Path) -> Dict[str, Any]:
    return run_subsystem_selftests(
        ...
        payload_builder=alias_audit_payload,
    )


def main() -> int:
    parser = argparse.ArgumentParser(description="Run the QDP subsystem S17 alias audit module.")
    ...
    if not args.case_id:
        raise SystemExit("S17 run requires --case-id unless --selftest is set.")
