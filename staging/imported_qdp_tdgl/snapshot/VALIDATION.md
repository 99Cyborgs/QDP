...
This runs the legacy repo-level candidate sweep over `artifacts/outputs/**/*_candidate.json`.
...
- run `tdgl-rf validate-phase2-4a validation/seeded_vortex_phase2_4a_validation_manifest.yaml` plus the targeted Phase-2.4A validation / experiment-sweep / CLI tests when the committed stochastic operator pack, fixed-seed validation manifest, fail-closed staging/promotion/aggregation behavior, or replay/provenance comparison surface changes,
...
- treat `validation/seeded_vortex_phase2_4a_experiment_pack.yaml` as the committed runtime driver for Phase-2.4A stochastic execution,
- treat `validation/seeded_vortex_phase2_4a_validation_manifest.yaml` as the committed same-stack Phase-2.4A validation surface over fixed-seed member identity, selected replay metadata, aggregate observables, and robustness summaries,
- treat all Phase-2.4A validation outputs as runtime/control-plane evidence only; current enforcement covers explicit noise opt-in, sequential fail-fast execution, staged promotion, strict full-member aggregation, and replay/provenance integrity, not broader scientific stochastic acceptance,
