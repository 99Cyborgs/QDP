...
4. **Stochastic noise / ensembles**
   This should be deferred until the deterministic baseline is stronger, the initialization surface is clearer, and the reporting/reproducibility contract is explicit. A committed runtime operator pack now exists for control-plane exercise, and a committed same-stack validation surface now exists for fixed-seed runtime/control-plane stability, but neither artifact satisfies the missing scientific acceptance surface. Otherwise the project will lose interpretability before it gains credible new claims.
