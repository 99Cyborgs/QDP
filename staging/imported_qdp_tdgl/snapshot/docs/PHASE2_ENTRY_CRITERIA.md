...
- The committed validation surface is `tdgl-rf validate-phase2-4a validation/seeded_vortex_phase2_4a_validation_manifest.yaml`, which reruns the fixed-seed operator pack and freezes stable same-stack control-plane outputs.
- The committed runtime operator pack `validation/seeded_vortex_phase2_4a_experiment_pack.yaml` remains the execution driver for that validation surface, but neither artifact widens the scientific claim boundary or provides grounded stochastic acceptance thresholds by itself.
