...
Run the committed Phase-2.4A stochastic seeded-vortex operator pack:

```bash
tdgl-rf run-experiment validation/seeded_vortex_phase2_4a_experiment_pack.yaml
```

Validate the committed Phase-2.4A fixed-seed same-stack surface:
...
- `tdgl-rf run-experiment validation/seeded_vortex_phase2_4a_experiment_pack.yaml` runs the committed Phase-2.4A stochastic same-stack operator pack with fixed sweep and ensemble settings.
- `tdgl-rf validate-phase2-4a validation/seeded_vortex_phase2_4a_validation_manifest.yaml` reruns that committed pack and freezes the stable semantic validation surface: fixed-seed member identity, selected replay metadata, aggregate observables, and robustness summaries.
...
- Phase-2.4A aggregate artifacts now also include runtime-only per-parameter-point robustness summaries over successful ensemble members.
- Phase-2.4A validation is same-stack runtime/control-plane validation only. It is not a scientific validation claim, and the repo does not currently define grounded acceptance guardrails for `noise.strength` beyond the explicit runtime contract `noise.enabled=true`, `noise.seed`, and `noise.strength > 0`.
