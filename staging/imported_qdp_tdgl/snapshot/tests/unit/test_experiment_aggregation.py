...
def test_aggregate_ensemble_results_includes_robustness_summary() -> None:
    ...
    point = payload["parameter_points"][0]
    robustness_summary = point["robustness_summary"]

    assert robustness_summary["member_count"] == 2
    assert robustness_summary["successful_member_count"] == 2
    assert robustness_summary["failed_member_count"] == 0
    assert robustness_summary["failure_rate"] == pytest.approx(0.0)
    assert robustness_summary["observables"]["early_window.mean_abs2_final"]["coefficient_of_variation"] == pytest.approx(0.1 / 0.9)
    assert robustness_summary["observables"]["early_window.mean_abs2_final"]["coefficient_of_variation_defined"] is True
    assert robustness_summary["observables"]["early_window.mean_abs2_final"]["sign_consistency"] == pytest.approx(1.0)
