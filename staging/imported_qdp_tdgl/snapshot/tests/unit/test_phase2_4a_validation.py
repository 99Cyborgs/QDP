...
from tdgl_rf.workflows.phase2_4a_validation import (
...
def _committed_expected_results() -> dict[str, object]:
    payload = yaml.safe_load(Path("validation/seeded_vortex_phase2_4a_validation_manifest.yaml").read_text(encoding="utf-8"))
    return payload["expected_results"]


def test_load_phase2_4a_validation_manifest_committed_surface() -> None:
    manifest = load_phase2_4a_validation_manifest("validation/seeded_vortex_phase2_4a_validation_manifest.yaml")
    ...
    assert Path(manifest.experiment_pack_path) == Path("validation/seeded_vortex_phase2_4a_experiment_pack.yaml").resolve()
...
def test_phase2_4a_top_level_mismatch_is_reported_at_exact_path() -> None:
    ...
    mismatches = _collect_exact_mismatches(expected, observed, surface="top_level", subject_id="overall")
...
def test_member_provenance_projection_excludes_path_dependent_fields() -> None:
    baseline = {
        ...
        "initialization": {
            ...
            "configured_vortex_seeds": [{"seed_index": 0, "winding": 1, "x0": 6.0, "y0": 3.0, "core_radius": None}],
            "resolved_vortex_seeds": [{"seed_index": 0, "winding": 1, "x0": 6.0, "y0": 3.0, "core_radius": 0.25, "plaquette_x": 11, "plaquette_y": 5}],
