...
def test_load_committed_phase2_4a_experiment_pack() -> None:
    ...
    assert pack.schema_version == "4.0"
    assert pack.mode == "stochastic_ensemble"
    assert pack.manifest_path == str(manifest_path)
    assert pack.base_case_path.endswith("canonical_single_positive_stochastic.yaml")
    assert pack.ensemble is not None
    assert pack.ensemble.member_count == 3
    assert pack.ensemble.master_seed == 1234
    assert [parameter.name for parameter in pack.sweep_parameters] == ["forcing.a_rf"]
    assert pack.observables == ["early_window.mean_abs2_final"]
