...
from tdgl_rf.workflows.phase2_4a_validation import Phase24AValidationSummary
...
def test_validate_phase2_4a_cli_uses_committed_default_manifest(monkeypatch, tmp_path: Path) -> None:
    ...
    def _fake_run(manifest_path, *, output_dir=None):
        ...
        return Phase24AValidationSummary(
            ...
            experiment_pack_path="validation/seeded_vortex_phase2_4a_experiment_pack.yaml",
    ...
    monkeypatch.setattr("tdgl_rf.cli.run_phase2_4a_validation", _fake_run)
    runner = CliRunner()

    result = runner.invoke(app, ["validate-phase2-4a", "--output-dir", str(tmp_path / "explicit_out")])

    assert result.exit_code == 0
    assert captured["manifest_path"] == Path("validation/seeded_vortex_phase2_4a_validation_manifest.yaml").resolve()
